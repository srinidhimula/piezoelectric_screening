from structureinfo import struct_nodeinfo_oxdnumber
...
for dirs in os.listdir('.'):
    ...
    if os.path.exists(path_piezo+'/results.txt'):
        ...
        os.chdir(path_piezo)
        try:
            df_structinfo,final_metalidx_uniq=struct_nodeinfo_oxdnumber("POSCAR",where="vasp") #Gives information about coordination around the metal
        except:
            df_structinfo,final_metalidx_uniq=struct_nodeinfo_oxdnumber(qmofpathdir+"CONTCAR",where="cif")
        ...
        df_structinfo.to_csv(maindir+"/structure_becoxdnuminfo_set123.csv",mode='a',header=False)
